...
export class GoogleSheetsService {
  ...
  async syncTable() {
    const { theCycleMembers, allMembers, maraphonUsers } =
      await this.userService.getUsersWithExportData();
    const webinar1Users =
      await this.liveEventsService.listRegistrationsForExport(WEBINAR_1_KEY);
    ...
    const webinar1Rows = webinar1Users.map((user) => {
      return [
        user.id,
        user.username || '',
        user.name || '',
        ...
        user.utm.campaigns || '-',
        user.supportRequests.length || 0,
        this.formatDate(user.userCreatedAt),
        user.status,
        this.formatDate(user.createdAt),
      ];
    });

    await this.httpService.axiosRef.post(this.googleSheetsWebhookUrl, {
      theCycle: cycleRows,
      all: allRows,
      maraphon4: maraphon4Rows,
      webinar1: webinar1Rows,
    });
